...
class UcZoneDropDownWidget extends StatelessWidget {
  const UcZoneDropDownWidget({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<UpdateCustomerCubit, UpdateCustomerState>(
      builder: (context, state,) {
        return  state.isLocalictyLoading ? const DropdownLoadingWidget() :  DropdownSearch<ZoneModel>(
                enabled: true,
                selectedItem: state.selectedZoneModel,

                dropdownBuilder: (context, selectedItems) => state.selectedZoneModel == null ?  const CommonTextWidget(title: "Select Zone",fontWeight: FontWeight.w500,textColor: Colors.grey,) : CommonTextWidget(title: state.selectedZoneModel!.zoneName.toString()) ,
