...
class DailyPlanCustomerListDropdownWidget extends StatelessWidget {
  const DailyPlanCustomerListDropdownWidget({super.key});
    ...
    return BlocBuilder<DailyPlanCubit, DailyPlanState>(
      builder: (context, state,) {
        return DropdownSearch<Customermodel>.multiSelection(
                ...
                dropdownBuilder: (context, selectedItems) => CommonTextWidget(title: selectedItems.isEmpty ? 'Select Customers' : selectedItems.map((e) => e.farm!.isIndividual == true ?  e.farm!.farmName == null ? 'no name found' : e.farm!.farmName.toString(): e.customerName ).join(' , '),fontWeight: selectedItems.isEmpty ? FontWeight.w400 : FontWeight.w500,maxLines: 5,) ,
                ...
                itemAsString: (item) => item.farm!.isIndividual == true ?  "${item.farm!.farmName} \n(${item.farm!.custLocation})\n(${item.customerName.toString()})" : item.customerName.toString(),                // show: (EmployeModel employee) => employee.name,
                onChanged: (values) {
